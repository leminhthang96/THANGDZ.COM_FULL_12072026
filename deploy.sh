...
sudo -u $REAL_USER pm2 start node_modules/next/dist/bin/next --name "thangdz-frontend" --cwd "$PROJECT_DIR/website_thangdz/frontend" -- start -p 3000
...
sudo -u $REAL_USER pm2 start node_modules/next/dist/bin/next --name "thangdz-admin" --cwd "$PROJECT_DIR/web_quantri_thangdz" -- start -p 3001
